...
| The urgent sound is distinct from the ordinary chime | ✅ structural test | `playRunRiskAlertSound()` is a six-pulse two-tone siren (880/1047 Hz) plus a 220 Hz body and a two-note tail — no pitch-sweeps at all, versus the chime's three sweeping raindrops. Still sine-only; peak 0.3 vs the chime's 0.24; its own 1.6s cooldown separate from the chime's 2.5s. `tools/reviews-feed-test.mjs` §14a–e. |
...
| The UI predicts the ruling | ❌ never | The banner prints only the observed call-on-field score and, when the payload supports it, the arithmetic score with the credited runs subtracted — otherwise that half is omitted, not invented. The banner, row badge and stat all carry an explicit "not a prediction" note. |
